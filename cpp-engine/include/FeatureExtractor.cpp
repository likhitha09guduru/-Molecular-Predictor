...
int FeatureExtractor::getFeatureCount() const
{
    return 8; // Number of features we extract
}
...
std::vector<double> FeatureExtractor::extract(RDKit::ROMol* mol)
{
    ...
    try {
        // Basic properties
        features.push_back(static_cast<double>(mol->getNumAtoms()));
        features.push_back(static_cast<double>(mol->getNumBonds()));
        ...
        // Rings
        features.push_back(static_cast<double>(mol->getRingInfo()->numRings()));
        ...
        features.push_back(static_cast<double>(heavy));
        ...
        features.push_back(static_cast<double>(donors));
        features.push_back(static_cast<double>(acceptors));
    } catch (...) {
        return features;
    }
